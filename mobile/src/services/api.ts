...
const API_BASE_URL = process.env.EXPO_PUBLIC_API_URL || 'https://api.lobster-network.ai/v4';
...
export const getBalance = (nodeId: string, currency?: string) => {
  return api.get(`/wallet/${nodeId}/balance`, { params: { currency } });
};

export const transfer = (nodeId: string, data: { to_node_id: string; amount: number }) => {
  return api.post(`/wallet/${nodeId}/transfer`, data);
};

// 节点 API
export const getNodes = (status?: string) => {
  return api.get('/node', { params: { status } });
};

export const registerNode = (data: { node_id: string; name: string; type: string }) => {
  return api.post('/node', data);
};

// 任务 API
export const getTasks = (status?: string) => {
  return api.get('/task', { params: { status } });
};

export const createTask = (data: { title: string; description: string; reward_amount?: number }) => {
...
export const getProposals = (status?: string) => {
  return api.get('/proposal', { params: { status } });
};

export const vote = (proposalId: string, data: { voter_id: string; option: string }) => {
