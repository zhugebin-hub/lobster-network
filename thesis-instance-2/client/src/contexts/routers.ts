...
export const appRouter = router({
  ...
  match: router({
    ...
    reviewWish: teacherProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        // 检查分流学生优先模式：如果激活且学生为非分流学生，且当前导师是中方导师，则不允许审批通过
        // 英文老师不受分流优先模式限制
        if (input.decision === "approved") {
          const priorityMode = await db.checkTransferStudentPriorityMode();
          if (priorityMode.isActive && ctx.user.teacherType === "chinese") {
            // 获取学生信息
            const student = await db.getUserById(wish.studentId);
            if (student && student.studentType !== "transfer") {
              throw new TRPCError({ 
                code: "FORBIDDEN", 
                message: "当前处于分流学生优先模式，中方导师无法审批通过非分流学生的志愿申请。请优先处理分流学生的申请。" 
              });
            }
          }
        }
  ...
  admin: router({
    ...
    setConfig: adminProcedure
      ...
      .mutation(async ({ input }) => {
        return db.setConfig(input.key, input.value, input.description);
      }),
    ...
    exportMatches: adminProcedure
      .mutation(async () => {
        ...
        // 论文选题来源, 论文研究方向 Research Interests, 论文撰写语种 Language, 成绩, 备注
        const headers = [
          "序号",
          "学院 Faculty",
          "专业 Major",
...
${rows.map(row => `<Row>${row.map(cell => `<Cell><Data ss:Type="String">${String(cell).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</Data></Cell>`).join("")}</Row>`).join("\n")}
    ...
    getTimeConfig: adminProcedure.query(async () => {
      ...
      return {
        ...
        topicPublishStart: timePhase.topicPublishStart?.toISOString() || "",
        topicPublishEnd: timePhase.topicPublishEnd?.toISOString() || "",
        studentSelectionStart: timePhase.studentSelectionStart?.toISOString() || "",
        studentSelectionEnd: timePhase.studentSelectionEnd?.toISOString() || "",
        teacherConfirmStart: timePhase.teacherConfirmStart?.toISOString() || "",
        teacherConfirmEnd: timePhase.teacherConfirmEnd?.toISOString() || "",
...
${rows.map(row => `<Row>${row.map(cell => `<Cell><Data ss:Type="String">${String(cell).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</Data></Cell>`).join("")}</Row>`).join("\n")}
    ...
    getCurrentPhase: protectedProcedure.query(async () => {
      ...
      return {
        ...
        topicPublishStart: timePhase.topicPublishStart?.toISOString() || null,
        topicPublishEnd: timePhase.topicPublishEnd?.toISOString() || null,
        studentSelectionStart: timePhase.studentSelectionStart?.toISOString() || null,
        studentSelectionEnd: timePhase.studentSelectionEnd?.toISOString() || null,
        teacherConfirmStart: timePhase.teacherConfirmStart?.toISOString() || null,
        teacherConfirmEnd: timePhase.teacherConfirmEnd?.toISOString() || null,
