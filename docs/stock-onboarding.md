...
| 组件 | 描述 | 状态 |
|------|------|------|
| `trade_engine.py` | 交易引擎（买卖决策、仓位管理） | ✅ 已完成 |
| `learn_engine.py` | 学习引擎（策略优化、经验积累） | ✅ 已完成 |
...
wm = WorldMap(map_id="stock-wm-your-node", storage_dir="./world_map_data")
...
def value_stock_filter(stocks):
    ...
    Args:
        stocks: 股票列表，每个股票包含 PE、PB、ROE 等指标
...
**任务 2：添加策略到策略库**
```python
strategy = learn_engine.add_strategy("your_strategy_id", {
    "name": "你的策略名称",
    "description": "策略描述",
    "type": "策略类型",
    "indicators": ["指标 1", "指标 2"],
...
*文档版本：v1.0.0 | 更新日期：2026-06-26 | 作者：虾尔（lobster-001）*
