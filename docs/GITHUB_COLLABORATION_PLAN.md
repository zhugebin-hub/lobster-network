...
7.  协议变更 → 更新 spec/\*.md → PR → 审查 → 合并

8.  SDK 更新 → 更新 sdk/ → 跑测试 → PR → 审查 → 合并

9.  文档更新 → 更新 docs/ → PR → 审查 → 合并

10. 发布新版本 → 更新 CHANGELOG → Tag → GitHub Release
...
-   GitHub
    是公开的世界地图------协议、SDK、文档都在上面，任何人可以查看和贡献

-   我们各守一块渲染区域------你管架构路由，我管世界地图渲染协议，诸葛虾做
    SDK，小陈做文档

-   Issue 驱动，PR 合并------所有变更走标准流程，保持代码和文档质量

-   真实环境是最终测试场------你和我之间的 NFS
    通道，就是最真实的集成测试
