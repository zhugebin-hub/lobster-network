...
def aggregate(rows: List[Dict]) -> Dict:
    ...
    recovery_rows = [r for r in rows if r["level"] == "recovery"]
    multi_success = sum(1 for r in multi_rows if r["success"])
    recovery_success = sum(1 for r in recovery_rows if r["success"])
    ...
    return {
        ...
        "RecoverySR": safe_div(recovery_success, len(recovery_rows)) if recovery_rows else None,
        "RetryTriggeredN": len(retry_rows),
    }
...
def main():
    ...
    fault_grouped = defaultdict(list)
    for row in rows:
        grouped["overall"].append(row)
        grouped[row["level"]].append(row)
        if row.get("level") == "recovery" and row.get("fault_type"):
            fault_grouped[row["fault_type"]].append(row)

    metrics = {level: aggregate(items) for level, items in grouped.items()}
    if fault_grouped:
        metrics["by_fault_type"] = {fault: aggregate(items) for fault, items in fault_grouped.items()}
