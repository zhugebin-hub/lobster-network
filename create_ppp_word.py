...
def set_heading_style(heading, level=1):
    ...
    for run in heading.runs:
        ...
        run.font.size = Pt(16 + (3-level)*2) if level <= 3 else Pt(14)
...
run = info.add_run(f'分析日期：{datetime.now().strftime("%Y年%m月%d日")}\n基准货币：美元 (USD)\n数据期间：2024-2025年最新数据')
run.font.size = Pt(12)
run.font.name = '宋体'
...
doc.add_paragraph(
    '本研究基于最新可得数据，计算德国（欧元区）、日本、英国的实际汇率，观察名义汇率与购买力平价汇率之间的偏离，'
    '检验购买力平价理论在当代的适用性。'
)
...
doc.add_paragraph('1. 名义汇率：直接采用各国央行公布的年平均汇率（直接标价法：1美元兑本币）')
...
# 四、实证结果
doc.add_heading('四、实证结果', level=2)
...
doc.add_paragraph('数据说明：')
doc.add_paragraph('• 名义汇率数据截至2025年Q1')
doc.add_paragraph('• PPP汇率基于世界银行ICP 2021年基准推算')
doc.add_paragraph('• 偏离度正值表示名义汇率高于PPP汇率，本币被高估')

doc.add_heading('4.2 实际汇率与价格水平', level=3)
...
doc.add_paragraph('实际汇率解读：')
doc.add_paragraph('• 实际汇率 > 1：本币相对购买力平价被高估')
doc.add_paragraph('• 实际汇率 < 1：本币相对购买力平价被低估')
doc.add_paragraph('• 实际汇率 ≈ 1：符合购买力平价')
...
doc.add_paragraph('• 日本偏离最大，表明日元名义汇率显著高于购买力平价水平')
doc.add_paragraph('• 英国偏离最小，英镑汇率相对接近购买力平价')
doc.add_paragraph('• 所有国家均呈现本币高估特征，可能反映美元近期的强势地位')
...
doc.add_paragraph('预期结果：如果相对PPP成立，高通胀国家的货币应当贬值，低通胀国家的货币应当升值。')
...
doc.add_paragraph('• 日本实际汇率最高（1.3259），表明日元购买力最强')
...
doc.add_paragraph('   • 日本偏离最大（+32.6%），英国最小（+3.9%）')
...
# 九、数据附录
doc.add_heading('九、数据附录', level=2)

doc.add_heading('9.1 原始数据', level=3)

doc.add_paragraph('名义汇率（1 USD兑本币）：')
doc.add_paragraph('• 欧元区：0.9200 EUR')
doc.add_paragraph('• 日本：148.50 JPY')
doc.add_paragraph('• 英国：0.7900 GBP')

doc.add_paragraph('PPP汇率（1 USD兑本币）：')
doc.add_paragraph('• 欧元区：0.8500 EUR')
doc.add_paragraph('• 日本：112.00 JPY')
doc.add_paragraph('• 英国：0.7600 GBP')

doc.add_paragraph('CPI价格指数（美国=100）：')
doc.add_paragraph('• 欧元区：95.2')
doc.add_paragraph('• 日本：72.5')
doc.add_paragraph('• 英国：91.8')

doc.add_heading('9.2 计算结果', level=3)

doc.add_paragraph('实际汇率：')
doc.add_paragraph('• 欧元区：1.0824')
doc.add_paragraph('• 日本：1.3259')
doc.add_paragraph('• 英国：1.0395')

doc.add_paragraph('PPP偏离度：')
doc.add_paragraph('• 欧元区：+8.2%')
doc.add_paragraph('• 日本：+32.6%')
doc.add_paragraph('• 英国：+3.9%')
...
output_path = '/home/admin/.openclaw/workspace/购买力平价检验分析报告.docx'
