...
center('浙江工商大学', size=28, bold=True, fname='黑体', sa=20)
...
center('课程论文', size=26, bold=True, fname='黑体', sa=30)
...
center('目\u3000录', size=16, bold=True, sb=20, sa=24)
...
toc_items = [
    ...
    ('        3.2.1  就业冲击', '', False),
    ('        3.2.2  数字鸿沟', '', False),
    ('        3.2.3  教育与再培训成本', '', False),
    ('    3.3  伦理与治理成本', '', False),
    ('        3.3.1  隐私与数据安全', '', False),
    ('        3.3.2  算法偏见与公平性', '', False),
    ('        3.3.3  责任与法律', '', False),
    ('    3.4  环境与能源成本', '', False),
    ('4  AI对大学生职业规划的影响', '5', True),
    ('    4.1  就业市场的结构性变化', '', False),
    ('        4.1.1  岗位替代与创造', '', False),
    ('        4.1.2  技能需求的转变', '', False),
    ('        4.1.3  工作方式的变革', '', False),
    ('    4.2  大学生职业规划的挑战', '', False),
    ('        4.2.1  职业选择的不确定性增加', '', False),
    ('        4.2.2  技能更新的持续性要求', '', False),
    ('        4.2.3  心理压力和焦虑', '', False),
    ('    4.3  面向AI时代的大学生职业规划策略', '', False),
    ('        4.3.1  培养跨学科复合能力', '', False),
    ('        4.3.2  提升数字素养和AI工具使用能力', '', False),
    ('        4.3.3  强化AI难以替代的核心能力', '', False),
    ('        4.3.4  制定灵活的职业发展路径', '', False),
    ('        4.3.5  重视实践经验和项目经历', '', False),
    ('5  高校职业规划教育的建议', '8', True),
    ('    5.1  将AI素养纳入职业规划课程体系', '', False),
    ('    5.2  建立跨学科培养机制', '', False),
    ('    5.3  强化实践教学和产教融合', '', False),
    ('    5.4  提供个性化的职业辅导服务', '', False),
    ('    5.5  关注学生心理健康', '', False),
    ('6  结论', '9', True),
    ('参考文献', '10', True),
]
...
for title, page, is_main in toc_items:
    row = table.add_row()
    # 左侧标题
    ...
    if is_main:
        ...
        r.font.size = Pt(11) if '    ' not in title or title.strip()[0].isdigit() else Pt(10.5)
    r.font.name = '宋体'
    r.element.rPr.rFonts.set(qn('w:eastAsia'), '黑体' if is_main else '宋体')
    # 右侧页码
    ...
    if page:
        r2 = p2.add_run(page)
        r2.font.size = Pt(12) if is_main else Pt(11)
        r2.font.name = '黑体' if is_main else '宋体'
        r2.bold = is_main
        r2.element.rPr.rFonts.set(qn('w:eastAsia'), '黑体' if is_main else '宋体')
