...
# 读取数据
input_file = '/home/admin/.openclaw/media/inbound/7555a0db-68fe-4286-bd84-5ce6eff6f6a1.xlsx'
output_dir = Path('/home/admin/.openclaw/workspace/stock-reports')
output_dir.mkdir(parents=True, exist_ok=True)

df = pd.read_excel(input_file)

# 数据清洗 - 排除异常数据（第4行全1但得分为0）
df_clean = df[df['学号'] != 122170106].copy()
print(f"原始数据：{len(df)} 人，清洗后：{len(df_clean)} 人（排除异常数据）")
...
**数据说明：** 共{len(df_clean)}名学生参与测试（排除 1 条异常数据），3 所学校，20 道选择题
