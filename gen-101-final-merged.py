#!/usr/bin/env python3
"""
"101计划"首批核心课程培育推进会汇报PPT - 最终融合版
融合：v3-final（研讨会+任务） + paper-based（论文理论+钉钉截图）
"""
...
# Left: Abstract
rect(s, 0.6, 1.3, 6.0, 2.8, C['light'], C['accent'], 2, True)
tb(s, 0.8, 1.4, 5.5, 0.5, '\u7814\u7a76\u6458\u8981', fs=18, c=C['primary'], b=True)
mt(s, 0.8, 1.9, 5.5, 2.0,
   ['\u4ee5\u6d59\u6c5f\u7701\u201cCS&AI 101\u8ba1\u5212\u201d\u4e3a\u80cc\u666f\uff0c\u63a2\u8ba8\u5982\u4f55\u5c06AI\u5de5\u5177\u878d\u5165\u6559\u5b66\u5b9e\u8df5\uff0c\u5b9e\u73b0\u4ece\u201c\u77e5\u8bc6\u704c\u8f93\u201d\u5411\u201cAI\u5e94\u7528\u5b9e\u8df5\u201d\u7684\u8303\u5f0f\u8f6c\u578b\u3002\u63d0\u51fa\u201c\u8bfe\u7a0b\u4f5c\u4e3aAI\u5e94\u7528\u80cc\u666f\u573a\u666f\u201d\u7406\u5ff5\uff0c\u6784\u5efa\u201c\u5b66\u751f\u4f5c\u4e1a\u5373\u8bfe\u7a0b\u8d44\u6e90\u201d\u6a21\u5f0f\uff0c\u901a\u8fc7SDN\u6848\u4f8b\u9a8c\u8bc1AI\u8fed\u4ee3\u601d\u7ef4\u5728\u6559\u5b66\u4e2d\u7684\u53ef\u884c\u6027\u3002'],
   fs=12, c=C['text'])

# Right: 101 Plan Background
rect(s, 7.0, 1.3, 5.8, 2.8, C['light'], C['secondary'], 2, True)
tb(s, 7.2, 1.4, 5.3, 0.5, '101\u8ba1\u5212\u80cc\u666f', fs=18, c=C['primary'], b=True)
mt(s, 7.2, 1.9, 5.3, 2.0,
   ['\u2022 \u6559\u80b2\u90e8\u7edf\u7b79\u7684\u62d4\u5c16\u521b\u65b0\u4eba\u624d\u57f9\u517b\u7b51\u57fa\u6027\u5de5\u7a0b',
    '\u2022 \u6c47\u805a\u9876\u5c16\u9ad8\u6821\u3001\u5e08\u8d44\u3001\u51fa\u7248\u5355\u4f4d',
    '\u2022 \u6a21\u5757\u4e00\uff1a\u8ba1\u7b97\u673a\u7f51\u7edc\uff08\u672c\u79d1\u00b7\u4e13\u4e1a\u6838\u5fc3\u8bfe\uff09',
...
# Bottom: Key data
rect(s, 0.6, 4.4, 12.2, 2.6, C['primary'], r=True)
tb(s, 0.8, 4.5, 3, 0.5, '\u73b0\u6709\u6210\u679c\u57fa\u7840', fs=18, c=C['accent'], b=True)

ach_data = [
    ('MOOC\u8bfe\u7a0b', '\u300a\u9ad8\u7ea7\u7f51\u7edc\u901a\u4fe1\u539f\u7406\u5b9e\u8df5\u300b\n228\u4eba\u5b66\u4e60 | 18\u5468 | \u963f\u91cc\u4e91\u5408\u4f5c'),
    ('\u7701\u7ea7\u5b9e\u9a8c\u8bfe', '\u300a\u8ba1\u7b97\u673a\u7f51\u7edc\u5b9e\u9a8c\u300b\n7\u671f | 623\u4eba\u6b21 | 22\u6821 | 59.4\u4e07\u8bbf\u95ee'),
    ('\u51fa\u7248\u6559\u6750', '\u300a\u7f51\u7edc\u901a\u4fe1\u539f\u7406\u5b9e\u8df5\u300b\uff08\u6e05\u534e,2024\uff09\n\u300a\u7cfb\u7edf\u7ea7\u7f16\u7a0b\u53ca\u5206\u5e03\u5f0f\u5e94\u7528\u300b\uff08\u5df2\u51fa\u7248\uff09'),
    ('\u5b9e\u9a8c\u5e73\u53f0', '\u963f\u91cc\u4e91\u4e91\u5b9e\u9a8c\u5ba4\nMininet/OpenDaylight/OpenStack'),
]
ax = 0.8
for title, desc in ach_data:
    rect(s, ax, 5.1, 2.8, 1.7, C['secondary'], r=True)
    tb(s, ax+0.1, 5.15, 2.6, 0.35, title, fs=13, c=C['white'], b=True, a=PP_ALIGN.CENTER)
    tb(s, ax+0.1, 5.55, 2.6, 1.2, desc, fs=10, c=C['white'], a=PP_ALIGN.CENTER)
    ax += 3.0

# ============================================================
# SLIDE 3: Theory Framework + Core Philosophy
# ============================================================
s = prs.slides.add_slide(prs.slide_layouts[6])
bg(s, C['white'])
hdr(s, '\u4e8c\u3001AI\u539f\u751f\u6559\u5b66\u8303\u5f0f\u6838\u5fc3\u7406\u5ff5')
...
px = 0.5
for title, color, content in pillars:
    rect(s, px, 1.3, 4.0, 5.6, color, r=True)
    tb(s, px+0.15, 1.4, 3.7, 0.5, title, fs=17, c=C['white'], b=True, a=PP_ALIGN.CENTER)
    mt(s, px+0.15, 2.0, 3.7, 4.7, content, fs=12, c=C['white'])
    px += 4.15

# ============================================================
# SLIDE 4: Innovation Model
# ============================================================
s = prs.slides.add_slide(prs.slide_layouts[6])
bg(s, C['white'])
hdr(s, '\u4e09\u3001\u521b\u65b0\u5b9e\u8df5\u6a21\u5f0f\uff1a\u201c\u5b66\u751f\u4f5c\u4e1a\u5373\u8bfe\u7a0b\u8d44\u6e90\u201d')

tb(s, 0.8, 1.4, 4, 0.5, '\u64cd\u4f5c\u6d41\u7a0b', fs=20, c=C['accent'], b=True)

steps = [
    ('\u2460 \u77e5\u8bc6\u70b9\u62c6\u89e3', '\u5c06\u8bfe\u7a0b\u77e5\u8bc6\u70b9\u62c6\u89e3\u4e3a\u5b66\u751f\u4f5c\u4e1a\u4efb\u52a1\uff0c\u6bcf\u8282\u8bfe1-2\u4e2a\u77e5\u8bc6\u70b9'),
    ('\u2461 \u5b66\u751f\u5b9e\u8df5', '\u5b66\u751f\u4f7f\u7528AI\u5de5\u5177\uff08Manus\u3001\u667a\u80fd\u4f53\u7b49\uff09\u751f\u6210\u52a8\u753b\u3001\u8bb2\u89e3\u89c6\u9891\u3001\u4ee3\u7801\u7b49'),
...
tb(s, 1, 5.55, 5, 0.4, 'SDN\uff08\u8f6f\u4ef6\u5b9a\u4e49\u7f51\u7edc\uff09\u8bfe\u7a0b\u5b9e\u8df5\u6848\u4f8b', fs=16, c=C['secondary'], b=True)
mt(s, 1, 5.95, 11, 1.0,
   ['\u2022 \u77e5\u8bc6\u70b9\u62c6\u89e3\uff1a\u5c06SDN\u6838\u5fc3\u6982\u5ff5\uff08\u63a7\u5236\u5668\u3001\u6570\u636e\u5e73\u9762\u3001OpenFlow\u534f\u8bae\u7b49\uff09\u62c6\u89e3\u4e3a10\u4e2a\u4f5c\u4e1a\u4efb\u52a1',
    '\u2022 \u4ece100\u4efd\u4f5c\u4e1a\u4e2d\u7b5b\u9009\u51fa30\u4efd\u4f18\u8d28\u4f5c\u54c1\uff0c\u5f62\u6210SDN\u77e5\u8bc6\u70b9\u8d44\u6e90\u5e93\uff08\u52a8\u753b/\u8bb2\u89e3/\u4ee3\u7801\u793a\u4f8b\uff09',
    '\u2022 \u5173\u952e\u539f\u5219\uff1a\u4e0d\u6ee1\u8db3\u4e8e\u201c\u770b\u4e0a\u53bb\u6f02\u4eae\u201d\uff0c\u7b26\u5408\u4e13\u4e1a\u6807\u51c6\uff0c\u5f62\u6210\u7cfb\u7edf\u5316\u8d44\u6e90\u5e93'],
   fs=13, c=C['text'])

# ============================================================
# SLIDE 5: Five Reform Measures
# ============================================================
s = prs.slides.add_slide(prs.slide_layouts[6])
bg(s, C['white'])
hdr(s, '\u56db\u3001\u4e94\u5927\u6539\u9769\u4e3e\u63aa')
...
ini = [
    ...
    { 'n': '03', 't': '\u5168\u6d41\u7a0b\u667a\u6167\u6559\u5b66\u7ba1\u7406',
      'd': 'MOOC\u6155\u8bfe\u5802+\u9489\u9489AI\u52a9\u7406\uff0c\u8bfe\u524d-\u8bfe\u4e2d-\u8bfe\u540e\u5168\u6d41\u7a0b\u8986\u76d6\u3002\u9489\u9489\u7fa4\u4e0eAI\u52a9\u624b\u534f\u4f5c\uff0c\u5b9e\u73b0\u4f1a\u8bae\u7eaa\u8981\u81ea\u52a8\u751f\u6210\u3001\u8bfe\u7a0b\u603b\u7ed3\u667a\u80fd\u6c47\u603b\u3002', 'c': C['accent'] },
    { 'n': '04', 't': '\u667a\u80fd\u4f53\u8f85\u52a9\u5b9e\u9a8c\u65b0\u8303\u5f0f',
      'd': 'AI\u8f85\u52a9\u751f\u6210\u547d\u4ee4+\u4eba\u5de5\u4e13\u4e1a\u6392\u9519+\u771f\u5b9e\u8bbe\u5907\u90e8\u7f72\u3002\u9a8c\u8bc1\u6027/\u8bbe\u8ba1\u6027/\u7efc\u5408\u6027=3:4:3\uff0c\u5b9e\u9a8c\u6548\u7387\u63d0\u534740%+\u3002', 'c': C['success'] },
    { 'n': '05', 't': '\u667a\u80fd\u4f53\u9a71\u52a8\u7684\u8bfe\u7a0b\u95e8\u6237\u5efa\u8bbe',
      'd': '\u8bfe\u7a0b\u5b98\u7f51\uff0c\u96c6\u6210\u6559\u5b66\u8d44\u6e90+\u5b9e\u9a8c\u6848\u4f8b+\u4f18\u79c0\u4f5c\u54c1+\u667a\u80fd\u4e92\u52a8\u3002\u5a92\u4f53\u5316\u8868\u8fbe\uff0c\u8d85\u8d8a\u4f20\u7edfPPT\u548c\u6559\u6750\u3002', 'c': C['purple'] }
]
...
# ============================================================
# SLIDE 6: DingTalk AI Management (with screenshot)
# ============================================================
s = prs.slides.add_slide(prs.slide_layouts[6])
bg(s, C['white'])
hdr(s, '\u4e94\u3001\u9489\u9489\u7fa4AI\u539f\u751f\u8bfe\u7a0b\u7ba1\u7406\u5b9e\u8df5')
...
# Screenshot
img_path = '/home/admin/.openclaw/workspace/dingtalk-group-screenshot.png'
if not os.path.exists(img_path):
    img_path = '/home/admin/.openclaw/workspace/dingtalk-group-full-screenshot.png'
if not os.path.exists(img_path):
    img_path = '/home/admin/.openclaw/workspace/dingtalk-chat-screenshot.png'
...
tb(s, 0.8, 1.4, 5, 0.5, '\u8bc4\u4f30\u6307\u6807\u5bf9\u6bd4', fs=18, c=C['primary'], b=True)
rect(s, 0.8, 2.0, 5.5, 0.4, C['primary'])
tb(s, 0.9, 2.0, 1.8, 0.4, '\u6307\u6807', fs=12, c=C['white'], b=True)
tb(s, 2.7, 2.0, 1.8, 0.4, '\u4f20\u7edf\u6a21\u5f0f', fs=12, c=C['white'], b=True)
tb(s, 4.5, 2.0, 1.8, 0.4, 'AI\u539f\u751f', fs=12, c=C['white'], b=True)
rows = [
    ('\u4f5c\u4e1a\u5b8c\u6210\u5ea6', '100%', '\u4e0d\u8981\u6c42\u5168\u5458'),
    ('\u4f5c\u54c1\u8d28\u91cf', '\u6559\u5e08\u4e3b\u89c2', '\u4f5c\u54c1\u8d28\u91cf\u9a71\u52a8'),
    ('\u8d44\u6e90\u79ef\u7d2f', '\u6559\u5e08\u4e3b\u5bfc', '\u5b66\u751f\u4f5c\u4e1a\u5373\u8d44\u6e90'),
...
for i, (c1, c2, c3) in enumerate(rows):
    ...
    rect(s, 0.8, ry, 5.5, 0.35, bg_c)
    tb(s, 0.9, ry, 1.8, 0.35, c1, fs=11, c=C['text'])
    tb(s, 2.7, ry, 1.8, 0.35, c2, fs=11, c=C['text'], a=PP_ALIGN.CENTER)
    tb(s, 4.5, ry, 1.8, 0.35, c3, fs=11, c=C['text'], a=PP_ALIGN.CENTER)
    ry += 0.38

# Challenges
tb(s, 7, 1.4, 5, 0.5, '\u6311\u6218\u4e0e\u5bf9\u7b56', fs=18, c=C['primary'], b=True)
challenges = [
...
for title, chal, sol in challenges:
    rect(s, 7, cy, 5.5, 1.0, C['light'], r=True)
    tb(s, 7.1, cy+0.02, 5.2, 0.25, title, fs=12, c=C['primary'], b=True)
    tb(s, 7.1, cy+0.28, 2.5, 0.65, f'\u6311\u6218\uff1a{chal}', fs=10, c=C['muted'])
    tb(s, 9.8, cy+0.28, 2.6, 0.65, f'\u5bf9\u7b56\uff1a{sol}', fs=10, c=C['text'])
    cy += 1.05

# Bottom: Key principle
rect(s, 0.8, 6.1, 11.5, 1.0, C['light'], C['accent'], 2, True)
mt(s, 1, 6.15, 11, 0.9,
   ['\u8bc4\u4f30\u539f\u5219\uff1a\u201c\u4e0d\u5728\u4e8eAI\u80fd\u5426\u5b8c\u6210\u4efb\u52a1\uff0c\u800c\u5728\u4e8e\'',
    '\u82b1\u66f4\u591a\u65f6\u95f4\u540e\u662f\u5426\u80fd\u505a\u5f97\u66f4\u597d\'\u3002\u201d\u2014\u2014 \u6559\u80b2\u5b9e\u6548\u4f18\u5148\uff0c\u8fed\u4ee3\u601d\u7ef4'],
   fs=14, c=C['primary'], italic=True)
...
# ============================================================
# SLIDE 11: Pilot + Collaboration
# ============================================================
s = prs.slides.add_slide(prs.slide_layouts[6])
bg(s, C['white'])
hdr(s, '\u5341\u3001\u8bd5\u70b9\u9ad8\u6821\u4e0e\u534f\u4f5c\u673a\u5236')

tb(s, 0.8, 1.4, 4, 0.5, '\u8bd5\u70b9\u9ad8\u6821\u540d\u5355\uff085\u6240\uff09', fs=18, c=C['primary'], b=True)
unis = ['\u6d59\u6c5f\u5de5\u4e1a\u5927\u5b66', '\u676d\u5dde\u7535\u5b50\u79d1\u6280\u5927\u5b66', '\u6d59\u6c5f\u7406\u5de5\u5927\u5b66', '\u5b81\u6ce2\u5927\u5b66', '\u6d59\u6c5f\u5e08\u8303\u5927\u5b66']
uy = 2.1
for u in unis:
    rect(s, 0.8, uy, 5.5, 0.55, C['light'], r=True)
    tb(s, 1.1, uy+0.05, 4, 0.45, f"\u2b50 {u}", fs=16, c=C['primary'], b=True)
    uy += 0.7

tb(s, 7.5, 1.4, 3, 0.5, '\u8bd5\u7528\u5185\u5bb9\u4e0e\u53cd\u9988', fs=18, c=C['primary'], b=True)
mt(s, 7.5, 2.0, 5, 2.5,
   ['\u2022 \u914d\u5957\u6559\u6750 + \u5b9e\u9a8c\u6307\u5bfc\u4e66',
    '\u2022 \u5728\u7ebf\u8bfe\u7a0b + \u5b9e\u8df5\u6848\u4f8b',
    '\u2022 \u4e91\u5e73\u53f0\u5b9e\u9a8c\u73af\u5883 + AI\u7b54\u7591\u6a21\u5757',
...
# Collaboration
rect(s, 0.8, 5.5, 11.5, 1.6, C['light'], r=True)
tb(s, 1, 5.55, 3, 0.4, '\u534f\u4f5c\u673a\u5236', fs=16, c=C['primary'], b=True)
mt(s, 1, 5.95, 11, 1.0,
...
rect(s, 2, 4.0, 9, 2.5, C['light'], r=True)
mt(s, 2.2, 4.1, 8.6, 2.3,
   ['\u57fa\u4e8eAI\u539f\u751f\u6559\u5b66\u8303\u5f0f\u7684\u8ba1\u7b97\u673a\u7f51\u7edc\u8bfe\u7a0b\u6539\u9769\u5b9e\u8df5',
    '',
...
out = '/home/admin/.openclaw/workspace/101-network-course-final-merged.pptx'
prs.save(out)
