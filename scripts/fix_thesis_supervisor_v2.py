...
def main():
    ...
    if ch1_start and ch1_end:
        ...
        for i in range(ch1_start, ch1_end):
            t = doc.paragraphs[i].text.strip()
            if '第一节' in t or '第二节' in t or '第三节' in t or '第四节' in t:
                sections.append(i)
        
        print(f"  找到{len(sections)}节")
        
        # 修改第四节标题（佛教中国化→其他宗教本土化经验）
        for i in range(ch1_start, ch1_end):
            t = doc.paragraphs[i].text.strip()
            if '佛教中国化' in t and ('节' in t or '佛教中国化的成功' in t):
                doc.paragraphs[i].text = '第四节 其他宗教本土化经验的参照意义'
                for run in doc.paragraphs[i].runs:
                    set_run(run, '宋体', 14, bold=True)
                print(f"  修改节标题: 其他宗教本土化经验的参照意义")
                break
        
        # 压缩佛教中国化相关内容
        for i in range(ch1_start, ch1_end):
            t = doc.paragraphs[i].text
            if '佛教中国化的成功经验为基督教中国化提供了参照' in t or '佛教中国化' in t:
                # 压缩这段
    ...
    if ch2_start and ch2_end:
        ...
        for i in range(ch2_start, ch2_end):
            t = doc.paragraphs[i].text.strip()
            if '第一节' in t or '第二节' in t or '第三节' in t or '第四节' in t or '第五节' in t:
                sections.append(i)
    ...
    if ch3_start and ch3_end:
        ...
        for i in range(ch3_start, ch3_end):
            t = doc.paragraphs[i].text.strip()
            if '第一节' in t or '第二节' in t or '第三节' in t or '第四节' in t or '第五节' in t or '第六节' in t:
                sections.append(i)
