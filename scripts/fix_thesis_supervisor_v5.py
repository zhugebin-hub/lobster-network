...
def main():
    ...
    # 根据实际结构修改
    # 第一章: 段落140-182 (3节: 141, 158, 172)
    # 第二章: 段落183-263 (3节: 184, 225, 247)
    # 第三章: 段落264-379 (3节: 265, 286, 318)
    # 第四章: 段落380-465 (4节: 382, 405, 425, 438)
    # 结语: 段落466-484
    # 参考文献: 段落485+
    ...
    # 修改"佛教中国化"相关内容为"其他宗教本土化经验"
    # 段落170左右是"佛教中国化的成功经验为基督教中国化提供了参照"
    for i in range(140, 183):
        ...
        if '佛教中国化的成功经验为基督教中国化提供了参照' in t:
            ...
            for run in doc.paragraphs[i].runs:
                set_run(run, '宋体', 12)
            print(f"  修改佛教中国化内容为其他宗教本土化经验 (段落{i})")
            break
    ...
    # 在各节结尾添加"当代转化提示"
    # 第一节结尾: 段落224 (第二节开始前)
    # 第二节(原第四节)结尾: 段落246 (第三节(原第五节)开始前)
    # 第三节(原第五节)结尾: 段落263 (第三章开始前)
    ...
    section_ends = [224, 246, 263]
    for idx, end_pos in enumerate(section_ends):
        if idx < len(transformation_tips) and end_pos < len(doc.paragraphs):
            para = doc.paragraphs[end_pos]
            para.text = transformation_tips[idx]
            for run in para.runs:
                set_run(run, '宋体', 12)
            print(f"  在第{idx+1}节结尾添加当代转化提示 (段落{end_pos})")
    ...
    section_starts = [265, 286, 318]
    for idx, sec_start in enumerate(section_starts):
        if idx < len(judgment_sentences) and sec_start + 1 < len(doc.paragraphs):
    ...
    section_starts = [382, 405, 425, 438]
    for idx, sec_start in enumerate(section_starts):
        if idx < len(chapter4_tips) and sec_start + 1 < len(doc.paragraphs):
            para = doc.paragraphs[sec_start + 1]
            current_text = para.text
