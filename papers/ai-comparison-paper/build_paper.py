#!/usr/bin/env python3
"""
按照浙江工商大学/双通班论文格式生成小论文 Word 文档
参考 thesis_final_v3.docx 的实际格式规范
"""
...
def add_run(paragraph, text, font_name='宋体', font_size=Pt(12), bold=False, align=None):
    """添加 Run 并设置格式"""
...
def add_title(text, font_size=Pt(22), bold=True):
    """居中标题"""
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p.paragraph_format.space_before = Pt(0)
    p.paragraph_format.space_after = Pt(0)
    add_run(p, text, '黑体', font_size, bold)
    return p


def add_blank_lines(n=2):
    """添加空行"""
    for _ in range(n):
        p = doc.add_paragraph()
        p.paragraph_format.space_before = Pt(0)
...
def add_info_line(label, value):
    ...
    p.paragraph_format.space_before = Pt(0)
    p.paragraph_format.space_after = Pt(0)
    add_run(p, label, '宋体', Pt(14))
    add_run(p, '  ' + value, '宋体', Pt(14))
    return p
...
def add_heading(text, level=1):
    ...
    p.paragraph_format.space_before = Pt(24) if level == 1 else Pt(12)
    p.paragraph_format.space_after = Pt(12) if level == 1 else Pt(6)
    p.paragraph_format.first_line_indent = Pt(0)
    font_size = Pt(16) if level == 1 else Pt(14)
    add_run(p, text, '黑体', font_size, True)
    return p
...
def add_table(table_data, col_widths=None):
    """添加表格"""
    rows = len(table_data)
    ...
    for i, row_data in enumerate(table_data):
        for j, cell_text in enumerate(row_data):
            ...
            cell.text = ''
            p = cell.paragraphs[0]
            add_run(p, str(cell_text), '宋体', Pt(10.5))
    return table
...
# ── 封面 ──
add_blank_lines(6)
add_title('当今主流人工智能系统的比较研究', Pt(22))
add_title('——功能特性、应用场景与用户体验分析', Pt(22))

add_blank_lines(6)
add_info_line('学    院：', '浙江省第二届宗教界"双通"人才研修班')
add_info_line('研究方向：', '宗教中国化与道教理论现代化')
add_info_line('姓    名：', '择白')
add_info_line('完成日期：', '2026年5月')
add_blank_lines(3)
...
add_run(p, '关键词：', '黑体', Pt(12), True)
add_run(p, '人工智能；大语言模型；ChatGPT；通义千问；比较研究；应用场景', '宋体', Pt(12))
...
# 第二章
add_heading('二、主流人工智能系统概述')

add_heading('（一）ChatGPT（OpenAI）', level=2)
...
add_heading('（二）Claude（Anthropic）', level=2)
...
add_heading('（三）通义千问（阿里巴巴）', level=2)
...
add_heading('（五）Kimi智能助手（月之暗面）', level=2)
...
# 第三章
add_heading('三、多维度比较分析')

add_heading('（一）技术能力比较', level=2)
add_heading('1. 语言理解与生成能力', level=3)

# 表1
add_body('表1  各系统语言理解与生成能力对比', indent=False)
p = doc.add_paragraph()
p.paragraph_format.space_before = Pt(0)
p.paragraph_format.space_after = Pt(6)
...
add_heading('2. 逻辑推理与数学能力', level=3)

add_body('表2  各系统逻辑推理与数学能力对比', indent=False)
p = doc.add_paragraph()
p.paragraph_format.space_before = Pt(0)
p.paragraph_format.space_after = Pt(6)
...
add_heading('3. 多模态能力', level=3)

add_body('表3  各系统多模态能力对比', indent=False)
p = doc.add_paragraph()
p.paragraph_format.space_before = Pt(0)
p.paragraph_format.space_after = Pt(6)
...
add_heading('（二）应用场景比较', level=2)

add_heading('1. 学术研究与论文写作', level=3)
...
add_heading('3. 日常办公与效率提升', level=3)
...
add_heading('（三）用户体验比较', level=2)

add_heading('1. 使用门槛', level=3)

add_body('表4  各系统使用门槛对比', indent=False)
p = doc.add_paragraph()
p.paragraph_format.space_before = Pt(0)
p.paragraph_format.space_after = Pt(6)
...
# 第四章
add_heading('四、典型使用场景案例分析')

add_heading('（一）学术论文写作辅助', level=2)
...
# 第五章
add_heading('五、存在的问题与挑战')

add_heading('（一）技术层面的问题', level=2)
...
add_heading('（三）中国AI发展的特殊挑战', level=2)
...
# 第六章
add_heading('六、未来发展趋势')

add_heading('（一）技术发展趋势', level=2)
