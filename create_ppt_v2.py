#!/usr/bin/env python3
"""生成南湖区中小学幼儿园后勤管理干部培训PPT v2（整合校舍维修+日常维护为第二章）"""
...
def add_bullet_list(slide, x, y, w, h, items, font_size=16, color=DARK_GRAY,
                    font_name='微软雅黑', line_spacing=1.7):
    txBox = slide.shapes.add_textbox(x, y, w, h)
    tf = txBox.text_frame
    tf.word_wrap = True
    ...
    for i, item in enumerate(items):
        ...
        for child in list(pPr):
            if child.tag.endswith('buChar') or child.tag.endswith('buNone'):
                pPr.remove(child)
        if item.strip() and not item.startswith('●') and not item.startswith('✗') and not item.startswith('①') and not item.startswith('②') and not item.startswith('③') and not item.startswith('④') and not item.startswith('⑤') and not item.startswith('⑥') and not item.startswith('⑦') and not item.startswith('⑧') and not item.startswith('⑨') and not item.startswith('⑩'):
...
def add_content_slide(slide, title, left_items=None, right_items=None, bottom_text=None, title_color=DARK_RED):
    ...
    if left_items and right_items:
        ...
        add_bullet_list(slide, Inches(0.7), y_start + Inches(0.2), Inches(5.5), Inches(5), left_items, font_size=16, line_spacing=1.7)
        add_bullet_list(slide, Inches(7.0), y_start + Inches(0.2), Inches(5.6), Inches(5), right_items, font_size=16, line_spacing=1.7)
    elif left_items:
        add_bullet_list(slide, Inches(0.8), y_start, Inches(11.5), Inches(5.5), left_items, font_size=18, line_spacing=1.8)
    if bottom_text:
        add_text_box(slide, Inches(0.8), Inches(7.0), Inches(11.5), Inches(0.4), bottom_text, font_size=12, color=MED_GRAY, alignment=PP_ALIGN.RIGHT)
...
add_text_box(slide, Inches(1.5), Inches(1.5), Inches(10), Inches(1.5),
             "南湖区中小学（幼儿园）", font_size=40, color=GOLD, bold=True, alignment=PP_ALIGN.CENTER)
add_text_box(slide, Inches(1.5), Inches(2.5), Inches(10), Inches(0.8),
             "后勤管理干部工作实务培训", font_size=48, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
add_text_box(slide, Inches(1.5), Inches(3.5), Inches(10), Inches(1.5),
             "校舍维修 · 日常维护 · 工程管理 · 装备采购 · 校服采购",
...
add_text_box(slide, Inches(1.5), Inches(6.3), Inches(10), Inches(0.5),
             "2026年6月", font_size=18, color=RGBColor(0xBB, 0xDE, 0xFB), alignment=PP_ALIGN.CENTER)
...
add_bg_rect(slide, Inches(0), Inches(1.1), Inches(13.333), Inches(0.06), GOLD)
add_text_box(slide, Inches(0.6), Inches(0.15), Inches(12), Inches(0.8),
             "目  录", font_size=32, color=WHITE, bold=True)
...
for i, (num, title) in enumerate(toc_items):
    ...
    add_text_box(slide, Inches(3.0), y + Inches(0.05), Inches(8), Inches(0.5),
                 title, font_size=22, color=DARK_GRAY)
...
add_content_slide(slide, "一、校园日常维护管理",
    ...
    bottom_text="南湖区要求：推行校园后勤管理标准化，建立'日巡查、周检查、月通报'制度")
...
# ==================== 第10页：食堂与食品安全 ====================
slide = prs.slides.add_slide(prs.slide_layouts[6])
add_content_slide(slide, "四、食堂与食品安全管理",
    left_items=[
        "食堂管理要求",
        "● 取得食品经营许可证",
        "● 落实校长陪餐制度",
...
add_text_box(slide, Inches(1.5), Inches(1.0), Inches(10), Inches(1.0),
             "总结与要求", font_size=44, color=WHITE, bold=True, alignment=PP_ALIGN.CENTER)
...
# ==================== 保存 ====================
output_path = "/home/admin/.openclaw/workspace/南湖区中小学幼儿园后勤管理干部培训_v2.pptx"
