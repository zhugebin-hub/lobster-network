...
def ssh_command(host: str, command: str) -> str:
    ...
    result = subprocess.run(full_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
...
def download_result(student_id: str, filename: str) -> Dict:
    ...
    subprocess.run(full_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
